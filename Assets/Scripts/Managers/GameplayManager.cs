...
public class GameplayManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            GameObject tutorialInstance = Instantiate(tutorial, Vector3.zero, Quaternion.identity);
            Time.timeScale = 0.0f;
            ServiceProvider.GetService<IInputReader>().SwitchPlayerMapState();
            tutorialInstance.GetComponent<VideoPlayerManager>().CloseCallback += () =>
            {
                Time.timeScale = 1;
                ServiceProvider.GetService<IInputReader>().SwitchPlayerMapState();
                Cursor.visible = false;
